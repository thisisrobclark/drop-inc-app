...
import { ShoppingCart, Leaf, ClipboardList, LayoutDashboard, LogOut } from 'lucide-react'
...
export default function Navbar() {
  ...
  const navItems = [
    { to: '/catalog', icon: Leaf, label: 'Catalog' },
        ...
        <div className="flex items-center gap-1">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.label}
              {item.badge ? (
                <span className="bg-lime-500 text-dark text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
                  {item.badge}
                </span>
              ) : null}
            </Link>
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl min-w-[60px] transition-colors ${
                isActive(item.to) ? 'text-lime-400' : 'text-gray-500'
              }`}
            >
              <div className="relative">
                <item.icon className="w-5 h-5" />
                {item.badge ? (
                  <span className="absolute -top-1.5 -right-2.5 bg-lime-500 text-dark text-[10px] font-bold px-1 py-0.5 rounded-full min-w-[16px] text-center leading-none">
                    {item.badge}
                  </span>
                ) : null}
              </div>
